  ...
  <div class="min-h-screen bg-gray-50">
        ...
        <div class="lg:col-span-1">
          <div class="bg-white rounded-lg shadow p-6">
            <div class="text-center">
              ...
              <!-- 用户基本信息 -->
              <h2 class="mt-4 text-xl font-semibold text-gray-900">{{ userInfo?.nickname }}</h2>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div class="bg-blue-50 rounded-lg p-4">
                <div class="flex items-center">
                  ...
                  <div>
                    <div class="text-2xl font-bold text-blue-600">{{ statistics.totalCourses || 0 }}</div>
              ...
              >
                <div class="flex items-start space-x-3">
                  ...
                  <div class="flex-1 min-w-0">
                    <h4 class="font-medium text-gray-900 truncate">{{ course.title || '课程标题' }}</h4>
                    <p class="text-sm text-gray-500 mt-1">{{ course.description || '暂无描述' }}</p>
                    <div class="mt-2 flex items-center justify-between">
                      <span class="text-xs px-2 py-1 rounded-full" 
                            :class="course.enrollStatus === 0 ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'">
                        {{ course.enrollStatus === 0 ? '已报名' : '未知状态' }}
                      </span>
                      <span class="text-xs text-gray-400">
                        {{ formatTime(course.enrollTime) }}
                      </span>
                    </div>
                  </div>
                </div>
    ...
    <el-dialog v-model="showAvatarUpload" title="更换头像" width="400px">
      <div class="text-center">
        ...
        >
          <img v-if="newAvatar" :src="newAvatar" class="avatar w-32 h-32 rounded-full object-cover mx-auto" />
      ...
      <template #footer>
        <span class="dialog-footer">
          ...
          <el-button type="primary" @click="updateAvatar" :loading="uploading">确定</el-button>
        </span>
      </template>
    </el-dialog>
  </div>
...
const userTypeClass = computed(() => {
  if (!userInfo.value || !userInfo.value.roles) return 'bg-gray-100 text-gray-800'
  ...
  try {
    ...
    for (const userCourse of userCourses) {
      try {
        ...
        if ((courseRes as any).code === 200 && (courseRes as any).data) {
          // 合并课程信息和用户课程关系信息
          courseDetails.push({
            ...(courseRes as any).data,
            userCourseInfo: userCourse,
            enrollStatus: userCourse.status,
            enrollTime: userCourse.createTime,
            courseId: userCourse.courseId
          })
        }
      } catch (error) {
        console.error(`获取课程${userCourse.courseId}详情失败:`, error)
        // 如果获取详情失败，至少保留基本信息
        courseDetails.push({
          id: userCourse.courseId,
          courseId: userCourse.courseId,
          title: `课程 ${userCourse.courseId}`,
          cover: '/default-course.png',
          userCourseInfo: userCourse,
          enrollStatus: userCourse.status,
          enrollTime: userCourse.createTime
        })
...
const loadData = async () => {
  ...
  try {
    ...
    const [statsRes, coursesRes, viewsRes] = await Promise.all([
      ...
      userApi.getMyCourseList({ page: 1, size: 4 }).catch(() => ({ records: [] })),
      userApi.getViewHistory({ page: 1, size: 5 }).catch(() => ({ records: [] }))
    ])
