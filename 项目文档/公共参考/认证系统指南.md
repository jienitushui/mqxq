...
@SaCheckRole(value = {"管理员", "教师"}, mode = SaMode.OR)
public Result<?> adminOrTeacher() {
    // 管理员或教师可访问
}
```

**角色类型（使用中文 role_name，非 role_code）：**
- `管理员`：管理员（对应 role_code: ADMIN）
- `教师`：教师（对应 role_code: TEACHER）
- `用户`：学生（对应 role_code: USER）

> 注意：`@SaCheckRole` 注解的值必须使用中文 `role_name`（"管理员"/"教师"/"用户"），因为 `StpInterfaceImpl.getRoleList()` 通过 `getUserRoleNames()` 查询的是 `role_name` 字段，而非 `role_code`。
