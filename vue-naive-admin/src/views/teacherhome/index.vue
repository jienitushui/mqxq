      ...
      <n-card class="ml-12 w-70%" title="✨ 欢迎使用 Vue Naive Admin 2.0">
        ...
        <footer class="mt-12 flex items-center justify-end">
          ...
          <n-button
            type="primary"
            class="ml-12"
            @click="$router.push('/teacherhome/features')"
          >
            新功能导航
          </n-button>
          <n-button
...
const loadTeacherStats = async () => {
  try {
    const [courseStats, studentStats, homeworkStats] = await Promise.all([
      api.getMyCourseStats(),
      api.getStudentStats(), 
      api.getHomeworkStats()
    ])
    
    teacherStats.value = {
      totalCourses: courseStats?.data?.totalCourses || 0,
      totalStudents: studentStats?.data?.totalStudents || 0,
      totalHomeworks: homeworkStats?.data?.totalHomeworks || 0,
      pendingHomeworks: homeworkStats?.data?.pendingHomeworks || 0
    }
  } catch (error) {
...
const loadLatestComments = async () => {
  try {
    const res = await api.getLatestComments(5)
...
const loadTopCourses = async () => {
  try {
    const res = await api.getTopCourses()
    console.log(res)
